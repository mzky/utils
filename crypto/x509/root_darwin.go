...
import (
	"bytes"
	"fmt"
	macOS "github.com/mzky/utils/crypto/x509/internal/macos"
	"os"
	"strings"
)
...
func loadSystemRoots() (*CertPool, error) {
	...
	for _, domain := range []macOS.SecTrustSettingsDomain{
		...
		for i := 0; i < macOS.CFArrayGetCount(certs); i++ {
			...
			if err != nil {
				if debugDarwinRoots {
					fmt.Fprintf(os.Stderr, "github.com/mzky/utils/crypto/x509: domain %d, certificate #%d: %v\n", domain, i, err)
				}
				continue
			}
			...
			if domain == macOS.SecTrustSettingsDomainSystem {
				...
				if err != nil {
					if debugDarwinRoots {
						fmt.Fprintf(os.Stderr, "github.com/mzky/utils/crypto/x509: trust settings for %v: %v\n", cert.Subject, err)
					}
					continue
				}
				if debugDarwinRoots {
					fmt.Fprintf(os.Stderr, "github.com/mzky/utils/crypto/x509: trust settings for %v: %d\n", cert.Subject, result)
				}
			}
			...
			default:
				if debugDarwinRoots {
					fmt.Fprintf(os.Stderr, "github.com/mzky/utils/crypto/x509: unknown trust setting for %v: %d\n", cert.Subject, result)
				}
			}
		}
	}
...
func sslTrustSettingsResult(cert macOS.CFRef) (macOS.SecTrustSettingsResult, error) {
	...
	if err != nil || trustSettings == 0 {
		if debugDarwinRoots && err != macOS.ErrNoTrustSettings {
			fmt.Fprintf(os.Stderr, "github.com/mzky/utils/crypto/x509: SecTrustSettingsCopyTrustSettings for SecTrustSettingsDomainUser failed: %s\n", err)
		}
	...
	if err != nil || trustSettings == 0 {
		...
		if debugDarwinRoots && err != macOS.ErrNoTrustSettings {
			fmt.Fprintf(os.Stderr, "github.com/mzky/utils/crypto/x509: SecTrustSettingsCopyTrustSettings for SecTrustSettingsDomainAdmin failed: %s\n", err)
		}
