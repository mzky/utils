...
func (d *digest) UnmarshalBinary(b []byte) error {
	if len(b) < len(magic) || string(b[:len(magic)]) != magic {
		return errors.New("github.com/mzky/utils/crypto/sha1: invalid hash state identifier")
	}
	if len(b) != marshaledSize {
		return errors.New("github.com/mzky/utils/crypto/sha1: invalid hash state size")
	}
	b = b[len(magic):]
